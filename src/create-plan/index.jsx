...
//     const FINAL_PROMPT = `Generate a career path and financial goal plan based on the following details: \n\nCareer Path: ${formData?.careerPath}\nSalary Expectation: ${formData?.salaryExpectation}\nCurrent Savings: ${formData?.savings}\nFinancial Goal: ${formData?.financialGoal}\nDeadline: ${formData?.deadline}`;

//     console.log('Generated Prompt:', FINAL_PROMPT); // Log the generated prompt

//     try {
//       const result = await chatSession.sendMessage(FINAL_PROMPT);
//       console.log('Gemini Response:', result?.response?.text()); // Log Gemini's response

//       // Log full response data for debugging if necessary
//       console.log('Full Gemini Result:', result);

//       setLoading(false);
//       SaveCareerPlan(result?.response?.text()); // Pass the response text to the SaveCareerPlan function
//     } catch (error) {
//       setLoading(false);
...
//     "steps": ["Gain experience in software development", "Build a portfolio", "Network with professionals"]
//   },
//   "financialGoalPlan": {
//     "goal": "Buy a house",
//     "deadline": "5",
...
//     "investmentStrategy": ["Invest in mutual funds", "Save in high-yield accounts"],
//     "additionalConsiderations": ["Down payment", "Property taxes"]
//   }
// }
...
//     // Validate form data
//     if (!formData?.careerPath || !formData?.salaryExpectation || !formData?.financialGoal || !formData?.savings || !formData?.deadline) {
//       toast('Please fill all details');
//       return;
//     }

//     // Validate that savings and salary are numeric
//     if (isNaN(formData.savings) || isNaN(formData.salaryExpectation)) {
//       toast('Please enter valid numbers for salary and savings');
//       return;
//     }

//     // Validate deadline is a positive number
//     if (isNaN(formData.deadline) || Number(formData.deadline) <= 0) {
//       toast('Please enter a valid deadline (greater than 0)');
//       return;
//     }
...
//   const FINAL_PROMPT = `
// Generate a career path and financial goal plan based on the following details:
// Career Path: ${formData?.careerPath}
// Salary Expectation: ${formData?.salaryExpectation}
// Current Savings: ${formData?.savings}
// Financial Goal: ${formData?.financialGoal}
...
//             }
//         ],
//         "skillDevelopment": ["string", "string"],
...
//       toast('Error saving career plan');
//     }
//   };
...
// import { Button } from '../components/ui/button';
// import { toast } from 'sonner';
// import { generateCareerAndFinancialPlan } from '@/service/AIModal'; // Your AI service
// import { useNavigate } from 'react-router-dom';
// import { AiOutlineLoading } from 'react-icons/ai';
// import { Dialog, DialogContent, DialogHeader, DialogDescription } from '@/components/ui/dialog';
// import { useGoogleLogin } from '@react-oauth/google';
...
//     try {
//       // Call generateCareerAndFinancialPlan to send the user data and get the response
//       const result = await generateCareerAndFinancialPlan(formData);
//       console.log('Gemini Response:', result);

//       // Extract only the career plan and financial plan data from the response
//       const careerPlanData = result?.response?.text(); // Assuming this returns plain text data

//       // Save career plan and financial data
//       await SaveCareerPlan(careerPlanData); // Pass only the relevant response data (text)
//     } catch (error) {
//       setLoading(false);
//       console.error('Error generating career plan:', error);
//       toast('Error generating career plan');
...
//       await setDoc(doc(db, 'CareerPlans', docId), {
//         userSelection: formData,
//         careerPlanData: careerPlanData,  // Save only the generated text
//         userEmail: user?.email,
//         id: docId,
//       });
//       setLoading(false);
